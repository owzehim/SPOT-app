 ...
 *   university: string,
 *   major: string,
 *   educationLevel: 'bachelor' | 'master' | 'alumni',
...
export async function registerMember(payload) {
  const {
    email, password,
    firstName, lastName, studentNumber, yearOfBirth,
    gender, countryOfOrigin, university, major,
    educationLevel, yearNumber
  } = payload;
  ...
  if (!userId) {
    throw new Error('Account was created but user ID is missing. Please contact support.');
  }
  ...
  if (memberError) {
    ...
    console.error('Members insert failed after auth signup:', memberError);
